...
void requester_generate(ModbusMaster *master) {
	...
	Uint16 * dataPtr =  (Uint16 *)&(master->coils);
//	Uint16 sizeOfMap = 0;
...
//		sizeOfMap = sizeof(master->coils);
	...
	{
    if(requester.addr < MAXIMUM_BLOCK_OBJECTS)
    {
      dataPtr = (Uint16 *)&(requesterValues000);
    }
    if(requester.addr >= 7*MAXIMUM_BLOCK_OBJECTS && requester.addr < 8*MAXIMUM_BLOCK_OBJECTS)
    {
      dataPtr = (Uint16 *)&(requesterValues700);
    }
	}
...
void requester_save(ModbusMaster *master) {
	...
	// Reference to MODBUS Data Map
	Uint16 *dataPtr = &requesterValues000[0];
...
//		sizeOfMap = sizeof(master->holdingRegisters);
	    if(requester.addr < MAXIMUM_BLOCK_OBJECTS)
	    {
	      dataPtr = (Uint16 *)&(requesterValues000[0]);
	      addressOffset = 0x000;
	    }
	    if(requester.addr >= 7*MAXIMUM_BLOCK_OBJECTS && requester.addr < 8*MAXIMUM_BLOCK_OBJECTS)
      {
	      dataPtr = (Uint16 *)&(requesterValues700[0]);
	      addressOffset = 7*MAXIMUM_BLOCK_OBJECTS;
      }
	}
				...
				requester.functionCode == MB_FUNC_READ_HOLDINGREGISTERS || requester.functionCode == MB_FUNC_READ_INPUTREGISTERS)
	{
		...
		for(i=0; i < (requester.totalData); i++) {
			Uint16 padding = i + requester.addr - addressOffset;
